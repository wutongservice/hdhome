...
public class BackDoorSettingsActivity extends PreferenceActivity implements OnPreferenceClickListener,
    ...
    private  static Runnable runnable = new Runnable(){
        ...
        public void run() {
            if (SEHomeUtils.DEBUG) {
                Log.d(SEHomeUtils.TAG, "Enn, it might be a cat, so ignore it ......." + mKnockCount);
            }
            resetKnockingCount();
        }
    };
    ...
    public static boolean knockAtBackDoor(Context context) {
        if (SEHomeUtils.DEBUG) {
            Log.d(SEHomeUtils.TAG, "Hei, someone is knocking at the back door......." + mKnockCount);
        }
    ...
    private static void openBackDoor() {
        if (SEHomeUtils.DEBUG) {
            Log.d(SEHomeUtils.TAG, "Hello, the back door is opening..." + mKnockCount);
        }
        try {
            ...
            e.printStackTrace();
            Log.e(SEHomeUtils.TAG, "openBackDoor exception.");
        }
    }
        ...
        final View.OnClickListener closeSceneEditor = new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (null != mSceneEditor) {
                    ...
                    sceneManager.getCurrentScene().setStatus(SE3DScene.STATUS_OPTION_MENU, false);
                }
            }
        };
    ...
    private static boolean onExtraMenuItemClick(Context context, int index) {
        switch (index) {
            case 0:
                SEHomeUtils.exportOrImportWallpaper(context);
                break;
            case 1:
                SEHomeUtils.tryImportWallpaper(context);
            default:
                break;
        }
        return false;
