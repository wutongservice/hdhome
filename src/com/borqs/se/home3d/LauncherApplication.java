...
public class LauncherApplication extends Application {
    ...
    public void onCreate() {
        ...
        SEHomeUtils.attachApplication(this);
        super.onCreate();
        if (SEHomeUtils.isCurrentPackage(getCurProcessName())) {
...
//            ACRA.getErrorReporter().setReportSender(new AcraSender(this, formKey));
            ...
            System.load(SEHomeUtils.PKG_LIB_PATH + "/libsefactory.so");
            System.load(SEHomeUtils.PKG_LIB_PATH +"/libse.so");
            if (mOldVersion != mCurVersion) {
                WeatherSettings.setGLSPromptStatus(this, false);
            }
            SettingsActivity.saveVersionCode(this, mCurVersion);
//            File file = new File(SEHomeUtils.SDCARD_PATH + "/.debug");
//            if (file.exists()) {
//                SEHomeUtils.DEBUG = true;
//            }
        ...
        MarketConfiguration.initLocalWallPaper(getApplicationContext(), WallpaperUtils.getWallpaperFolder(SEHomeUtils.SDCARD_PATH));
        ...
        MarketUtils.setLogVisibility(BackDoorSettingsActivity.isDebugSuggestion(getApplicationContext()));
    }
        ...
        if (!file.exists() || mOldVersion == 0 || mCurVersion != mOldVersion) {
            try {
                ...
                if (SEHomeUtils.DEBUG)
                    Log.d(SEHomeUtils.TAG, "initDictionary: copy " + path + rt);
                is.close();
