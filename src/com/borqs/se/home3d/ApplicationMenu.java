...
//    private AppSearchPane mAppSearchPane;
    ...
    public ApplicationMenu(SE3DScene scene, String name) {
        super(scene, name);
    ...
    private TransParas getBackgroundTransparas() {
    ...
    private TransParas getIconSlotTransParas(int left, int top) {
        TransParas transparas = new TransParas();
    ...
    private TransParas getFaceSlotTransParas(int face) {
        TransParas transparas = new TransParas();
        float angle = face * 90;
    ...
    public void show(final int type, final DockObject dockObject) {
        if (!getScene().getStatus(SE3DScene.STATUS_APP_MENU) && mHasLoaded) {
            mDisableTouch = true;
            stopAllAnimation(null);
            getScene().setStatus(SE3DScene.STATUS_APP_MENU, true);
            ...
            if (mDockObject == null && getScene() instanceof SE3DHomeScene) {
                SE3DHomeScene homeScene = (SE3DHomeScene) getScene();
//                mDesk = homeScene.getDesk();
    ...
    private void setCubeToRightPosition() {
        stopAllAnimation(null);
        if (getScene() instanceof SE3DHomeScene) {
    ...
    public void hide(boolean fast, final SEAnimFinishListener l) {
        if (getScene().getStatus(SE3DScene.STATUS_APP_MENU) && !mDisableTouch) {
            stopAllAnimation(null);
            mDisableTouch = true;
                    ...
                    if (mDockObject != null) {
                        mDockObject.show(new SEAnimFinishListener() {
                            public void onAnimationfinish() {
                                ...
                                mDisableTouch = false;
                                getScene().setStatus(SE3DScene.STATUS_APP_MENU, false);
                            }
                        });
                    } else {
                        if (l != null) {
    ...
    public boolean handleBackKey(SEAnimFinishListener l) {
        if (getScene().getStatus(SE3DScene.STATUS_APP_MENU)) {
            hide(false, l);
            return true;
        }
        return false;
    ...
    private class PreViewCylinder extends SEObjectGroup {
        ...
        public PreViewCylinder(SE3DScene scene, String name) {
            super(scene, name);
            mHasShowed = false;
        }
    ...
    private class PreViewFace extends SEObjectGroup {
        ...
        public PreViewFace(SE3DScene scene, String name) {
            super(scene, name);
        }
    ...
    public class PreViewObject extends SEObject {
        ...
        private PreViewObject(SE3DScene scene, String name, ItemInfo itemInfo, int w, int h) {
            super(scene, name);
...
//        }
    ...
    private class DocMenuGroup extends SEObjectGroup {
        ...
        public DocMenuGroup(SE3DScene scene, String name) {
            super(scene, name);
        }
...
//    }
    ...
    private void caculateAppsMenuIconSize() {
        mFontScale = SEHomeUtils.getFontSize(getContext());
        Resources res = getContext().getResources();
//        mScreenDensity = getSceneInfo().getScreenDensity();
    ...
    public Bitmap getAppMenuIconBitmap(Drawable icon, String title) {
        ...
        int newW = SEHomeUtils.higherPower2(bitmapW);
        int newH = SEHomeUtils.higherPower2(bitmapH);
