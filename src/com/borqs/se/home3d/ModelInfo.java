...
public class ModelInfo {
    ...
    public void createMenuInstanceForMaxModel(SE3DScene scene) {
        if (null == mMenuInstance) {
    ...
    public ObjectsMenuItem createMenuInstance(SE3DScene scene, SEObjectGroup parent) {
        if (null == mMenuInstance) {
            mMenuInstance = new ObjectsMenuItem(scene, this);
            if (Type.ICON_BOX.equalsIgnoreCase(mType)) {
    ...
    public static ModelInfo CreateFromXml(XmlPullParser parser) throws XmlPullParserException, IOException {
        ...
        if (previewTrans != null) {
            info.mPreviewTrans = new TransParas();
        ...
        if (localTrans != null) {
            info.mLocalTrans = new TransParas();
...
//        info.mAssetsPath = parser.getAttributeValue(null, ModelColumns.ASSETS_PATH);
    ...
    public static ModelInfo CreateFromDB(Cursor cursor) {
        ...
        if (previewTrans != null) {
            info.mPreviewTrans = TransParas.parseFrom(previewTrans);
        }
        String localTrans = cursor.getString(cursor.getColumnIndexOrThrow(ModelColumns.LOCAL_TRANS));
        if (localTrans != null) {
            info.mLocalTrans = TransParas.parseFrom(localTrans);
        }
    ...
    public void load3DMAXModel(final SE3DScene scene) {
        ...
        mNativeResource = SE3DScene.loadResource_JNI(mAssetsPath + "/" + mSceneFile, mAssetsPath + "/" + mBasedataFile);
    }
    ...
    public void add3DMAXModel(SE3DScene scene, SEObject parent) {
...
//            SEObject parentObj = scene.findObject(parent.mName, 0);
    ...
    public static SEObject getSkyObject(SE3DScene scene) {
        SEObject object = getVesselObject(scene, ClassName.SkyObject);
        return object;
    }
    public static HouseObject getHouseObject(SE3DScene scene) {
        SEObject object = getVesselObject(scene, ClassName.HouseObject);
        return (HouseObject)object;
    }
    public static DockObject getDockObject(SE3DScene scene) {
    ...
    private static Map<SE3DScene, VesselMap> sceneVesselObjectReference =
            new HashMap<SE3DScene, VesselMap>();
    private static SEObject getVesselObject(SE3DScene scene, String className) {
...
//                                    || "IconBox".equals(type);
    ...
    public static String getHouseType() {
        SE3DScene scene = SESceneManager.getInstance().getCurrentScene();
        if (null == scene) {
