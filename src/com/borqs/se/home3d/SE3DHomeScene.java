...
import com.borqs.se.scene.SESceneManager.ModelChangeCallBack;
...
public class SE3DHomeScene extends SE3DScene implements UnlockScreenListener, ModelChangeCallBack, LanguageChangeCallBack {
...
//    private IconPageIndicator mIndicator;
    public SE3DHomeScene(String sceneName) {
        super(sceneName);
        mMenuCheckStatus = SE3DScene.STATUS_APP_MENU + SE3DScene.STATUS_HELPER_MENU + SE3DScene.STATUS_OBJ_MENU
                + SE3DScene.STATUS_ON_WIDGET_SIGHT + SE3DScene.STATUS_MOVE_OBJECT + SE3DScene.STATUS_DISALLOW_TOUCH
                + SE3DScene.STATUS_ON_WALL_DIALOG;
    }
    ...
    private void loading() {
        ...
        if (rootVesselInfo != null) {
            NormalObject root = SEHomeUtils.getObjectByClassName(SE3DHomeScene.this, rootVesselInfo);
...
//        calculateCameraRadiusScope();
    ...
    private void onObjectShelfLoaded() {
        ...
        initScreenIndicator();
        createAndLoadMenu();
        LauncherModel.getInstance().addLanguageChangeCallBack(SE3DHomeScene.this);
        mLoadCompleted = true;
    }
...
//            finalizeOptionMenu();
            if ((mMenuCheckStatus & getStatus()) == 0) {
                SESceneManager.getInstance().runInUIThread(new Runnable() {
                    @Override
                    public void run() {
                        if (getStatus(SE3DScene.STATUS_OPTION_MENU)) {
                            BackDoorSettingsActivity.showOptionMenu(mContext, false);
                            setStatus(SE3DScene.STATUS_OPTION_MENU, false);
                        } else {
                            BackDoorSettingsActivity.showOptionMenu(mContext, true);
                            setStatus(SE3DScene.STATUS_OPTION_MENU, true);
                        }
                    }
                });
            }
        ...
        if (mOptionMenu != null) {
            if ((mMenuCheckStatus & getStatus()) == 0) {
                handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_OPTION_MENU_DIALOG, null);
            }
        }
...
//            }
    ...
    private void initHelperPreview() {
        mHelperPreview = new HelperMenu(SE3DHomeScene.this, "helper_preview");
        getContentObject().addChild(mHelperPreview, true);
    }

    private void initApplicationMenu() {
        mWidgetsPreview = new ApplicationMenu(SE3DHomeScene.this, "WidgetsPreview");
    ...
    private View.OnClickListener wallOptionItemClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			if(v.getId() == R.id.option_menu1) {
                new SECommand(SE3DHomeScene.this) {
                    public void run() {
                    	SEHomeUtils.initLocalObject(mContext);
...
//                    	handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_OBJECT_VIEW, null);
				...
				handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_APP_VIEW, ApplicationMenu.TYPE_SHOW_SHORTCUT);
                ...
                SESceneManager.getInstance().startActivityForResult(selectIntent,
                        SE3DHomeScene.REQUEST_CODE_BIND_WIDGET);
			}else if(v.getId() == R.id.option_menu5) {
				handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_SELECT_WALLPAPER_DIALOG, mWallPaperMsg);
			}
	 ...
	 private View.OnClickListener wallPaperSelectedListener = new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(v.getId() == R.id.option_menu1) {
					...
					Utils.resetWallpaper(getContext(), SE3DHomeScene.this, mCurrentTheme, mCurrentImage);
				}
			}
		};

	private WallLongClickDialog mWallContextMenu;
    ...
    private void getWallLongClickDialog() {
    	mSESceneManager.runInUIThread(new Runnable() {
            public void run() {
                ...
                {
                	if(mWallContextMenu == null) {
                		mWallContextMenu = new WallLongClickDialog(mSESceneManager.getGLActivity(), wallOptionItemClickListener);
                	}            	            	
...
//                        FileURLInfoColumns.LOCAL_PATH + " = '" + localPath + "'", null);
    ...
    private void closeFloatView() {
        ...
        if(activity instanceof SEHomeActivity) {
        	((SEHomeActivity)activity).closeFloatViews();
        }
    }
        ...
        if (!"fangdajing".equals(mSelectedObject.mName)) {
            if (mSelectedObject.canChangeBind()) {
                ...
                changeBindBtn.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        dialog.dismiss();
                        handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_BIND_APP_DIALOG, mSelectedObject);
                    }
                });
            ...
            uninstallBtn.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (mSelectedObject.getObjectInfo().isDownloadObj()) {
                        handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_DELETE_OBJECTS, mSelectedObject.mName);
                    } else {
...
//                Log.d(SEHomeUtils.TAG, "calculateCameraRadiusScope, minRadius = " + minRadius +
