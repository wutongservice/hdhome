...
import com.borqs.se.download.market.MarkChangerReceiver;
import com.borqs.se.download.market.MarkChangerReceiver.selectObjectListener;
import com.borqs.se.download.market.Utils;
...
import com.borqs.se.home3d.LauncherApplication;
...
import com.borqs.se.scene.SESceneManager.UnlockScreenListener;
...
//    private static final String WU_DING_OBJ_NAME_2 = "wuding02@group_house8@home8_basedata.cbf";
    ...
    public House(SE3DScene scene, String name, int index) {
        super(scene, name, index);
        MarkChangerReceiver.registerSelectObjectListener(getClass().getName(), this);
    ...
    */
    ...
    private void changeImage() {
        ...
        if (imageCursor != null) {
            while (imageCursor.moveToNext()) {
                ...
                if(!TextUtils.isEmpty(imageItem.mNewPath) && imageItem.mNewPath.startsWith(SEHomeUtils.SDCARD_PATH)) {
    ...
    private void loadImageItemOneByOne(final SE3DScene scene, final int index, final List<ImageItem> imageItems) {
...
//    	    assert(CEIL_LIGHT_OBJ_NAME != null);
    ...
    private void addWallObjects() {
        ...
        ground.setOnLongClickListener(new OnTouchListener() {
            public void run(SEObject obj) {
                ...
                getScene().handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_SELECT_WALLPAPER_DIALOG, msg);
            }
        });
        ...
        for (String wallName : WALL_OBJNAMES) {
    		...
    		obj.setOnLongClickListener(new OnTouchListener() {
    			public void run(SEObject obj) {
    				...
    				getScene().handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_WALL_LONG_CLICK_DIALOG, msg);
    			}
    		});
    ...
    */
    ...
    /*
    private TransParas getObjectTransParas(SERay ray, float radius, House house) {
        float skyRadius = getScene().mSceneInfo.mSkyRadius;
        TransParas transParas = rayCrossWall(ray, radius);
    ...
    */
    ...
    private TransParas getMountPointTransParas(ObjectInfo objectInfo, NormalObject currentObject) {
        ...
        final String wall0Name = WALL_OBJNAMES.get(mPreFaceIndex);
        SEObject wall = findChild(childName, 0);
        SEObject wall0 = findChild(wall0Name, 0);
        	...
        	if(mountPointIndex >= 0) {
                ...
                if(objectInfo.mType != null && objectInfo.mType.equals(ModelInfo.Type.WALL_SHELF)) {
                    ...
                    if (null == mpd) {
                        ...
                        SEHomeUtils.reportError(errorMsg.toString());
    ...
    public TransParas getSlotTransParas(ObjectInfo objectInfo, NormalObject object) {
        if (ModelInfo.isUsingMountPointSlot(objectInfo)) {
            Log.d(SEHomeUtils.TAG, "initStatus, object type: " + objectInfo.mName);
            final TransParas transParas = getMountPointTransParas(objectInfo, object);
            if (null != transParas) {
                return transParas;
            } else {
        ...
        if (objectInfo.mObjectSlot.mSpanX > 0 && objectInfo.mObjectSlot.mSpanY > 0) {
            ...
            if (objectInfo.mIsNativeObject) {
                TransParas localTrans = objectInfo.mModelInfo.mLocalTrans;
                if (localTrans != null)
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_DOWN:
            ...
            if (isBusy() || getScene().getStatus(SE3DScene.STATUS_ON_SKY_SIGHT)
                    || getScene().getStatus(SE3DScene.STATUS_ON_DESK_SIGHT)) {
                if (getScene().getStatus(SE3DScene.STATUS_ON_DESK_SIGHT)) {
                    getCamera().moveToWallSight(null);
                    mHasGotAction = true;
                } else if (getScene().getStatus(SE3DScene.STATUS_ON_SKY_SIGHT)) {
                    mOnMoveSight = true;
                    mHasGotAction = true;
                }
                return true;
...
//                }
    ...
    private class WallVelocityAnimation extends CountAnimation {
        ...
        public WallVelocityAnimation(SE3DScene scene, float velocity) {
            super(scene);
    ...
    private class ToFaceAnimation extends CountAnimation {
        ...
        public ToFaceAnimation(SE3DScene scene, float face, float step, boolean circle) {
            super(scene);
            mStep = step;
            mFace = face;
            mCircle = circle;
    ...
    private boolean performDoubleTap() {
        if (getScene().getStatus(SE3DScene.STATUS_ON_UNLOCK_ANIMATION) 
    ...
    class RunACircleAnimation extends CountAnimation {
        ...
        public RunACircleAnimation(SE3DScene scene) {
            super(scene);
        }
    ...
    public void onActivityDestory() {
        ...
        MarkChangerReceiver.unregisterSelectObjectListener(getClass().getName());
                ...
                if (v != null && v.getTag() != null) {
                    ...
                    new SECommand(getScene()) {
                        public void run() {
                            ...
                            TransParas startTranspara = new TransParas();
    ...
    public void unlockScreen() {
        boolean disable = getStatus(SE3DScene.STATUS_APP_MENU) | getStatus(SE3DScene.STATUS_HELPER_MENU)
                | getStatus(SE3DScene.STATUS_OPTION_MENU) | getStatus(SE3DScene.STATUS_OBJ_MENU)
                | getStatus(SE3DScene.STATUS_ON_SKY_SIGHT) | getStatus(SE3DScene.STATUS_ON_WIDGET_SIGHT)
                | getStatus(SE3DScene.STATUS_ON_WALL_DIALOG);
        if (!disable) {
            if (!getStatus(SE3DScene.STATUS_ON_DESK_SIGHT)) {
                new SECommand(getScene()) {
                    public void run() {
                        runACircle(null);
                    }
    ...
    public void shortcutAction(Context context, Intent data) {
        ...
        if (LauncherApplication.ACTION_INSTALL_SHORTCUT.equals(data.getAction())) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
        case SE3DHomeScene.REQUEST_CODE_BIND_WIDGET:
            if (data != null) {
            ...
            break;
        case SE3DHomeScene.REQUEST_CODE_SELECT_WIDGET:
            if (data != null) {
            ...
            break;
        case SE3DHomeScene.REQUEST_CODE_SELECT_SHORTCUT:
            ...
            break;
        case SE3DHomeScene.REQUEST_CODE_SELECT_WALLPAPER_IMAGE:
            if (resultCode == Activity.RESULT_OK) {
                final ImageItem imageItem = new ImageItem();
                imageItem.mImageName = mCurrentImage;
                String path = SEHomeUtils.PKG_FILES_PATH + mCurrentTheme;
...
//                        options.inSampleSize = SEHomeUtils.computeSampleSize(options, -1,  1024 * 1024);
...
//
//                        int size = SEHomeUtils.higherPower2(w);
//                        if (size > 1024) {
//                            size = 1024;
//                        }
...
//                        bm.recycle();
//                        SEHomeUtils.saveBitmap(des, imageItem.mNewPath, Bitmap.CompressFormat.JPEG);
//                        des.recycle();
            ...
            break;
        case SE3DHomeScene.REQUEST_CODE_SELECT_WALLPAPER_CAMERA:
            if (resultCode == Activity.RESULT_OK) {
                ...
                mSESceneManager.startActivityForResult(intent2, SE3DHomeScene.REQUEST_CODE_SELECT_WALLPAPER_IMAGE);
            }
            break;
        }
    }
    ...
    private void setAppWidget(int appWidgetId) {
        ...
        if (providerInfo.configure != null) {
            ...
            if (configureIntent != null) {
                mSESceneManager.startActivityForResult(configureIntent, SE3DHomeScene.REQUEST_CODE_SELECT_WIDGET);
            }
        } else {
            finishSetAppWidget(appWidgetId);
        }
    ...
    private void finishSetAppWidget(int appWidgetId) {
        ...
        if (providerInfo != null) {
            ...
            info.mSceneName = getScene().mSceneName;
            info.mType = "Widget";
            final int[] span = SEHomeUtils.getSpanForWidget(getContext(), providerInfo);
    ...
    private Bundle prepareWallpaper(SEObject obj, int type) {
        ...
        if (mSdcardTempFile == null) {
            File dir = new File(SEHomeUtils.SDCARD_PATH);
            if (!dir.exists()) {
                dir.mkdirs();
            }
            mSdcardTempFile = new File(SEHomeUtils.SDCARD_PATH + "/" + ".tempimage");
        }
        mCurrentImage = obj.getImageName_JNI();

        Bundle msg = new Bundle();
        msg.putString(SE3DHomeScene.MSG_CONTENT_IMAGE, mCurrentImage);
        msg.putString(SE3DHomeScene.MSG_CONTENT_IMAGE_OUTPUT, Uri.fromFile(mSdcardTempFile).toString());
        msg.putInt(SE3DHomeScene.MSG_CONTENT_IMAGE_SIZE_X, mImgSizeX);
        msg.putInt(SE3DHomeScene.MSG_CONTENT_IMAGE_SIZE_Y, mImgSizeY);
        return msg;
    }
...
//        ModelInfo modelInfo = scene.mSceneInfo.findModelInfo(objectName);
		...
		final NormalObject newObject = SEHomeUtils.getObjectByClassName(scene, objInfo);
