...
public class Laptop extends NormalObject implements UnlockScreenListener {
    ...
    public Laptop(SE3DScene scene, String name, int index) {
        ...
        setOnClickListener(new SEObject.OnTouchListener() {
            public void run(SEObject obj) {
                if (mCurrentShellAngle < 5) {
                    getScene().handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_APP_VIEW, ApplicationMenu.TYPE_SHOW_KEEP_LAST);
                } else {
    ...
    class OpenOrCloseAnimation extends CountAnimation {
        ...
        public OpenOrCloseAnimation(SE3DScene scene, float step, boolean openOrClose) {
            super(scene);
            mIsOpen = openOrClose;
            if (mIsOpen) {
