...
public class AppObject extends NormalObject {
    ...
    public AppObject(SE3DScene scene, String name, int index) {
    ...
    private TransParas getBackgroundLocation() {
        TransParas transparas = new TransParas();
        transparas.mTranslate.set(-9, SEHomeUtils.ICON_BACKGROUND_SPACING, 0);
    ...
    public Folder changeToFolder() {
        ...
        Folder folder = (Folder) SEHomeUtils.getObjectByClassName(getScene(), objInfo);
        ...
        TransParas transParas = folder.getFirstPosition();
...
//                int h = (int) houseObject.getWallUnitSizeY() * getObjectInfo().getSpanY();
                ...
                if (resolveInfo == null) {
                	Bitmap icon ; 
                	if(getObjectInfo().mComponentName != null && SEHomeUtils.LOCKSCREEN_HOMEHD_PKG.equals(getObjectInfo().mComponentName.getPackageName())) {
                		//TODO
    ...
    public static AppObject create(SE3DScene scene, AppItemInfo itemInfo) {
...
//        int h = houseObject.getWallUnitSizeY() * info.getSpanY();
    ...
    public boolean update(SE3DScene scene) {
        ...
        ResolveInfo resolveInfo = SEHomeUtils.findResolveInfoByComponent(scene.getContext(), info.mComponentName);
        if (resolveInfo == null) {
            return false;
        }
//        HouseObject houseObject = ModelInfo.getHouseObject(getScene());
...
//        final Bitmap icon = AppItemInfo.getBitmap(resolveInfo, w, h);
    ...
    private void startApplicationUninstallActivity(ComponentName componentName) {
        ...
        try {
            ...
            if (SEHomeUtils.DEBUG)
                Log.d(SEHomeUtils.TAG, "uninstall : " + e.getMessage());
        }
    }
...
//        }
//
//        int padding = (int) (SEHomeUtils.ICON_PADDING * scale);
