...
public class Television extends WallNormalObject {
    ...
    public Television(SE3DScene scene, String name, int index) {
    ...
    private class UpdateAnimation extends CountAnimation {
        ...
        public UpdateAnimation(SE3DScene scene) {
            super(scene);
        }
    ...
    private class PlayVideoThumbnails extends CountAnimation {
        ...
        public PlayVideoThumbnails(SE3DScene scene) {
            super(scene);
        }
        ...
        private void changeImage(final String name) {
            LoadResThread.getInstance().process(new Runnable() {
                public void run() {
                    mImageData = SEObject.loadImageData_JNI(SEHomeUtils.THUMB_PATH + "/" + name);
                    mHasLoadImage = true;
                }
            });
        }
        ...
        if (!TextUtils.isEmpty(mCurrentImageName)) {
            ...
            if (!TextUtils.isEmpty(path)) {
                ...
                try {
                    ...
                    Log.e(SEHomeUtils.TAG, "Thumbnails can not find the activity to play video.");
                }
            }
        }
        return false;
