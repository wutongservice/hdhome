...
public class WallDialog extends SEObjectGroup {
    ...
    public WallDialog(SE3DScene scene, String name) {
        super(scene, name, 0);
        mDisableTouch = true;
    ...
    public void show(Bundle wallpaperMsg, int touchX , int touchY) {
        if (!getScene().getStatus(SE3DScene.STATUS_ON_WALL_DIALOG)) {
            mDisableTouch = true;
            getParent().setUseUserColor(0.3f, 0.3f, 0.3f);
            stopAllAnimation(null);
            init(wallpaperMsg);
            getScene().setStatus(SE3DScene.STATUS_ON_WALL_DIALOG, true);
    ...
    public void hide(boolean fast, final SEAnimFinishListener l) {
        if (getScene().getStatus(SE3DScene.STATUS_ON_WALL_DIALOG)) {
            ...
            if (fast) {
                ...
                getScene().setStatus(SE3DScene.STATUS_ON_WALL_DIALOG, false);
                releaseChilds();
                if (l != null) {
                    l.onAnimationfinish();
                }
    ...
    private class ShowAnimation extends CountAnimation {
        ...
        private int mTouchX;
        private int mTouchY;
        public ShowAnimation(SE3DScene scene, int touchX, int touchY) {
            super(scene);
            mTouchX = touchX;
            mTouchY = touchY;
        }
    ...
    private class HideAnimation extends CountAnimation {
        ...
        public HideAnimation(SE3DScene scene) {
            super(scene);
    ...
    private void selectApp() {
        getScene().handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_APP_VIEW, ApplicationMenu.TYPE_SHOW_APP);
    }

    private void selectWallpaper(Bundle wallpaperMsg) {
        getScene().handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_SELECT_WALLPAPER_DIALOG, wallpaperMsg);
    }
}
