...
public class NormalObject extends SEObjectGroup {
    private TransParas mStartTranspara;
    ...
    */
    ...
    public NormalObject(SE3DScene scene, String name, int index) {
    ...
    public boolean update(SE3DScene scene) {
        return true;
    }
...
//                    && !mObjectInfo.mModelInfo.mType.equals("walldialog")) {
    ...
    public TransParas getAbsoluteTransParas() {
        TransParas retTranspara = new TransParas();
    ...
    public final void calculateNativeObjectTransParas() {
        ...
        if (mObjectInfo.mIsNativeObject) {
            TransParas origin = mObjectInfo.mModelInfo.mLocalTrans;
            if (origin != null) {
                TransParas localTrans = origin.clone();
    ...
    public void initStatus(SE3DScene scene) {
        ...
        if (null != vessel) {
            TransParas transParas = vessel.getSlotTransParas(getObjectInfo(), this);
            if (transParas != null) {
        ...
        setOnClickListener(new SEObject.OnTouchListener() {
            public void run(SEObject obj) {
                if (getObjectInfo().mSlotType == ObjectInfo.SLOT_TYPE_DESKTOP) {
                    handOnClick();
                } else if (!getScene().getStatus(SE3DScene.STATUS_ON_DESK_SIGHT)
    ...
    public void setOnMove(boolean onFinger) {
        if (onFinger) {
            ...
            getScene().setStatus(SE3DScene.STATUS_MOVE_OBJECT, true);
            getScene().setTouchDelegate(this);
            setPressed(true);
            getUserTransParas().set(getStartTranspara());
            if (getObjectInfo().mIsNativeObject) {
                TransParas localTrans = getObjectInfo().mModelInfo.mLocalTrans;
                if (localTrans != null) {
            ...
            setPressed(false);
            getScene().setStatus(SE3DScene.STATUS_MOVE_OBJECT, false);
    ...
    public boolean onMoveEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_MOVE:
        	...
        	if (!mPosChanged) {
        		...
        		if(mPosChanged) {
        			getScene().handleMessage(SE3DHomeScene.MSG_TYPE_DISMISS_OBJECT_LONG_CLICK_DIALOG, this);
        		}
            }
...
//                getScene().handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_BIND_APP_DIALOG, this);
    ...
    private void showBindAppDialog() {
    	if (SEHomeUtils.DEBUG)
            Log.d(SEHomeUtils.TAG, "not find intent, show binding dialog");
        if (mObjectInfo.mIsNativeObject) {
            getScene().handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_BIND_APP_DIALOG, this);
        }
    }
    ...
    public void handleOutsideRoom() {
        ...
        if (isFresh()) {
            ...
            if (getObjectInfo().isDownloadObj()) {
                getScene().handleMessage(SE3DHomeScene.MSG_TYPE_SHOW_DELETE_OBJECTS, this.mName);
            } else {
...
//        return typeManager.canPlaceOnVessel(mName, ObjectInfo.SLOT_TYPE_WALL);
    ...
    public void setStartTranspara(TransParas transParas) {
        mStartTranspara = transParas;
    }

    public TransParas getStartTranspara() {
    ...
    private class FlyAnimation extends CountAnimation {
        ...
        public FlyAnimation(SE3DScene scene, float velocityX, float velocityY, float step) {
            super(scene);
