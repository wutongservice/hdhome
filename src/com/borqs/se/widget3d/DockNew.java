...
//    private float mDeskTranslateZ;
    ...
    private TransParas mAttachObjectTransform;

    private static int mDrawerIndex = 2;

    public DockNew(SE3DScene scene, String name, int index) {
...
//        mAngle = 0;
    ...
    public void initStatus(SE3DScene scene) {
        ...
        if(mAttachObjectTransform == null) {
	    	    mAttachObjectTransform = new TransParas();
...
//            mDeskVelocityAnimation.execute();
    ...
    public void unlockScreen() {
        boolean disable = getStatus(SE3DScene.STATUS_APP_MENU) | getStatus(SE3DScene.STATUS_HELPER_MENU)
                ...
                | getStatus(SE3DScene.STATUS_ON_UNLOCK_ANIMATION)
                | getStatus(SE3DScene.STATUS_ON_WALL_DIALOG);
        if (!disable) {
            getScene().setStatus(SE3DScene.STATUS_ON_UNLOCK_ANIMATION, true);
            SEAnimFinishListener deskFinishListener = new SEAnimFinishListener() {
                public void onAnimationfinish() {
                    getScene().setStatus(SE3DScene.STATUS_ON_UNLOCK_ANIMATION, false);
                    mDisableTouch = false;
                }
            };
