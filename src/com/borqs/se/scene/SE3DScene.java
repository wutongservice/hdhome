...
import com.borqs.se.event.SEEventQueue;
...
import com.borqs.se.object3d.SEObject.SEObjectTravel;
...
public abstract class SE3DScene {
    private static final boolean DEBUG = true || SEHomeUtils.DEBUG;
    ...
    private SE3DScene mStartScene;
    private List<SE3DScene> mChildScenes;
    ...
    public void startScene(SE3DScene scene) {
        mStartScene = scene;
        create_JNI();
    ...
    public void addChildScene(SE3DScene scene) {
    ...
    public void removeChildScene(SE3DScene scene) {
        if (mChildScenes.contains(scene)) {
...
//        mSECamera.setFov(SECameraData.mBestCameraFov);
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (SEHomeUtils.DEBUG && mDownHitObject != null) {
                Log.d(SEHomeUtils.TAG, "Hit object name = " + mDownHitObject.mName + "; index = "
    ...
    public final void showDialog(final int id) {
        mSESceneManager.runInUIThread(new Runnable() {
            public void run() {
            	if (((SEHomeActivity)mSESceneManager.getGLActivity()).isLiving()) {
