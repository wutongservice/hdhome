...
import com.borqs.se.LocalService;
...
import com.borqs.se.home3d.SERenderView;
...
import com.borqs.se.weather.LoopForInfoService;
...
public class SESceneManager {
    ...
    public static final String BACKGROUND_IMAGE_PATH = SEHomeUtils.PKG_FILES_PATH + "/catch_scene.data";
    ...
    private List<SE3DScene> mScenes;
    private String mCurrentSceneName;
    private SE3DScene mCurrentScreen;
    ...
    private SESceneManager(Context context) {
        ...
        loadPlugins_JNI(SEHomeUtils.PKG_LIB_PATH);
        ...
        mScenes = new ArrayList<SE3DScene>();
        ...
        if (SEHomeUtils.DEBUG) {
            LoadResThread.getInstance().process(mDetectMemoryTask, 10000);
            setDebug_JNI(true);
        }
    ...
    private static class MemoryWatchDog {
        ...
        public void debugOutput() {
            ...
            Log.d(SEHomeUtils.TAG, "#### memory usage :" + value);
        }

        public void debugOutput(String msg) {
            final float privateDirty = getMyMemoryInfo().getTotalPrivateDirty() / 1024f;
            String value = "TotalPrivateDirty :" + privateDirty;
            Log.d(SEHomeUtils.TAG, msg + " #### memory usage :" + value);
        }
    ...
    private Runnable mDetectMemoryTask = new Runnable() {
        public void run() {
            ...
            LoadResThread.getInstance().cancel(mDetectMemoryTask);
            LoadResThread.getInstance().process(mDetectMemoryTask, 10000);
        }
    };
    ...
    public static SESceneManager getInstance() {
        return getInstance(LauncherApplication.getInstance().getApplicationContext());
    }
    ...
    public void addScene(SE3DScene scene) {
        if (!mScenes.contains(scene)) {
            mScenes.add(scene);
        }
    }

    public void removeScene(SE3DScene scene) {
    ...
    private void softRemoveScene(SE3DScene scene) {
        mScenes.remove(scene);
        if (scene != null) {
            scene.softRelease();
        }
    ...
    public SE3DScene findScene(String name) {
        for (SE3DScene scene : mScenes) {
    ...
    public void catchBackground() {
        int newW = SEHomeUtils.higherPower2(mWidth);
        int newH = SEHomeUtils.higherPower2(mHeight);
        int x = -((newW - mWidth) >> 1);
        int y = -((newH - mHeight) >> 1);
        glReadPixels_JNI(BACKGROUND_IMAGE_KEY, x, y, SEHomeUtils.higherPower2(mWidth),
                SEHomeUtils.higherPower2(mHeight));
    }
    ...
    public boolean startActivityForResult(Intent intent, int requestCode) {
        ...
        if (result & mCurrentScreen != null) {
            mCurrentScreen.setStatus(SE3DScene.STATUS_DISALLOW_TOUCH, true);
        }
        return result;
    }
    ...
    public SE3DScene getCurrentScene() {
        return mCurrentScreen;
    }

    private SE3DScene getDefaultScene() {
    ...
    public synchronized void start3DScene(String sceneName) {
        ...
        try {
            if (cursor != null && cursor.moveToFirst()) {
                ...
                SE3DScene screen = SEHomeUtils.getSceneByClassName(sceneInfo.mClassName, sceneInfo.mSceneName);
...
//    	removeScene(mCurrentScreen);
    	...
    	SEHomeDataBaseHelper help = SEHomeDataBaseHelper.getInstance(mGLActivity);
