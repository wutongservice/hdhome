...
public class SECamera {
    ...
    public SECamera(SE3DScene scene) {
...
//    }
    ...
    private static CountAnimation playNavigationAnimation(SE3DScene scene, SECamera camera,
...
//        });
    ...
    public static class NavigationAnimation extends CountAnimation {
        ...
        public NavigationAnimation(SE3DScene scene, SECamera camera,
                                   SECameraData src, SECameraData dst,
                                   float ratio) {
            super(scene);
...
//        if (mScene.getStatus(SE3DScene.STATUS_ON_UNLOCK_ANIMATION)) {
//            return;
//        }
//        if (breakAnimation) {
//            stopAllAnimation();
...
//        }
//        if (mSkyY == 0) {
//            mScene.setStatus(SE3DScene.STATUS_ON_DESK_SIGHT, false);
//            mScene.setStatus(SE3DScene.STATUS_ON_SKY_SIGHT, false);
//        } else if (mSkyY < 0) {
//            mScene.setStatus(SE3DScene.STATUS_ON_DESK_SIGHT, true);
//            mScene.setStatus(SE3DScene.STATUS_ON_SKY_SIGHT, false);
//        } else {
//            mScene.setStatus(SE3DScene.STATUS_ON_DESK_SIGHT, false);
//            mScene.setStatus(SE3DScene.STATUS_ON_SKY_SIGHT, true);
//        }
...
//        private float mStep;
//
//        public SetFovAnimation(SE3DScene scene, SECameraData endFov, float step) {
//            super(scene);
...
//    }
    ...
    private CountAnimation performMovieSightAnimation(SE3DScene scene, float dstY,
    ...
    public class MoveSightAnimation extends CountAnimation {
        ...
        public MoveSightAnimation(SE3DScene scene, float desSight, float step) {
            super(scene);
            mDesSkyY = desSight;
            mStep = step;
        }
    ...
    class UnlockScreenAnimation extends CountAnimation {
        ...
        public UnlockScreenAnimation(SE3DScene scene, int count) {
            super(scene);
            mCount = count;
            mStartFov = 45f;
...
//    	}
    ...
    private static SECameraData calculateTargetData(SECameraData originData, SECameraData finalData, float ratio) {
        if (ratio == Float.NaN) {
            Log.w(SEHomeUtils.TAG, "calculateTargetData, skip with Float.NaN: ratio = " + ratio);
            return null;
        } else if (originData.equals(finalData)) {
            Log.w(SEHomeUtils.TAG, "calculateTargetData, skip with identical origin and target.");
            return null;
        ...
        if (SEHomeUtils.DEBUG) {
            Log.d(SEHomeUtils.TAG, "calculateTargetData, enter data = " + target.toString()
        ...
        if (SEHomeUtils.DEBUG) {
            Log.d(SEHomeUtils.TAG, "calculateTargetData, calculated target = " + target.toString()
                    + ", ratio = " + ratio);
        }
        return target;
    }
