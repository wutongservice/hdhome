package com.borqs.se.weather.yahoo;
...
import com.borqs.se.weather.State;
import com.borqs.se.weather.WeatherController;
...
public class YahooLocationHelper {
    ...
    public static String parserWOEIDData(Document docWeather, Bundle bundle) {
        ...
        double latitude = bundle.getDouble(State.BUNDLE_LATITUDE);
        double longitude = bundle.getDouble(State.BUNDLE_LONGITUDE);
        try {
            ...
            for (int i = 0; i < placeNodes.getLength(); i++) {
                ...
                double tLongitude = Double.parseDouble(placeChildElement.getElementsByTagName("longitude").item(0)
                        .getTextContent());
                double tempDis = Math.abs(latitude - tLatitude) + Math.abs(longitude - tLongitude);
